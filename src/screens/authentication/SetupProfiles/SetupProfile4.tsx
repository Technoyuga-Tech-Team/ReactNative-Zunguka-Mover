      ...
      refetch().then(currentUser => {
        if (currentUser?.data?.user) {
          currentUser?.data?.user?.insurance_copies.length > 0 &&
            setSelectedImage(currentUser?.data?.user?.insurance_copies);
          // currentUser?.data?.user?.insurance_copies.length > 0 &&
          //   setImage(currentUser?.data?.user?.insurance_copies);

          let arr: any = [];
          if (currentUser?.data?.user?.insurance_copies.length > 0) {
            currentUser?.data?.user?.insurance_copies.map(ele => {
              arr.push({
      ...
      if (image.length > 0) {
        ...
        if (userSetupProfile.fulfilled.match(result)) {
          if (result.payload?.data) {
            if (result.payload?.data?.steps_count == 4) {
              navigation.navigate(Route.navSetupProfile5);
            }
          }
        } else {
